...
def get_location(location_str):
    ...
    latitude, longitude, status = dbOperations.check_coordinates_in_db(location_str)
    if status == 0:
        ...
        try:
            ...
            if location:
                dbOperations.insert_coordinates_to_db(location_str, location.latitude, location.longitude)
...
def get_location_using_google_maps(location_str, api_key):
    ...
    try:
        ...
        if result:
            ...
            dbOperations.insert_coordinates_to_db(location_str, latitude, longitude)
            return latitude, longitude
        else:
...
def check_ems_stat(current_incident_id, incident_location):
    ...
    surrounding_incidents = dbOperations.fetch_incident_window(current_incident_id, window_size=3)

    # Filter incidents by location
    incidents_at_same_location = [incident for incident in surrounding_incidents if incident[3] == incident_location]

    # Check the ORI for the current and surrounding incidents at the same location
    for incident in incidents_at_same_location:
...
def augment_data():
    with open('augmented_incidents.csv', mode='w', newline='') as file:
        pass  
    nature_rankings=dbOperations.get_nature_rankings()
    location_rankings=dbOperations.get_location_rankings()
    ...
    with open('augmented_incidents.csv', mode='w', newline='') as file:
        ...
        for row in incidents:
            ...
            logger.log_message(f"{day_of_week}\t{time_of_day}\t{weather}\t{location_rank}\t{side_of_town}\t{incident_rank}\t{nature}\t{int(emsstat)}")
            print(f"{day_of_week}\t{time_of_day}\t{weather}\t{location_rank}\t{side_of_town}\t{incident_rank}\t{nature}\t{int(emsstat)}")
            writer.writerow([day_of_week, time_of_day, weather, location_rank, side_of_town, incident_rank, incident_nature, int(emsstat)])